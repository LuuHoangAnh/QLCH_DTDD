...
namespace QLCHDTDD
{
    public partial class TimKiemMatHang : Form
    {
        ...
        {
            DataTable dtResult = ConnectDB.TimKiemMatHang(MaMH.Text.Trim().ToUpper());

            if (dtResult != null && dtResult.Rows.Count > 0 && dtResult.Rows[0] != null)
            {
                // Bạn có thể thực hiện hành động cụ thể ở đây
                dgvTimkiemmathang.DataSource = ConnectDB.TimKiemMatHang(MaMH.Text.Trim().ToUpper());
            }
            else
            {
                MessageBox.Show("Mã mặt hàng không hợp lệ!!", "Thông báo");
            }
        }
